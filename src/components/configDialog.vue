...
import BaseGraph from "@/views/config/baseGraph";
import { gridConfig } from "@/views/config/baseOther"
...
const activeName = ref('globalConfig')
const state = reactive(JSON.parse(JSON.stringify(gridConfig)))
const reset = () => {
    Object.assign(state, JSON.parse(JSON.stringify(gridConfig)))
    handleConfigChange()
}
